...
# ---- Training Hierarchical Model ----
    ...
    ckpt_path = "vae_hierarchical_large.pt"
    ...
    for epoch in range(start_epoch, hierarchical_epochs):
        ...
        tf_ratio = 0   # 0 low teacher forcing for aggressive AR
    
        print(f"--- Fine-tune Epoch {epoch+1}/{hierarchical_epochs} (KL={kl_weight:.3f}, TF={tf_ratio:.3f}) ---")
        ...
        avg_recon = total_recon / len(full_train_dataloader)
        avg_kl = total_kl / len(full_train_dataloader)
        ...
        avg_test_recon = test_recon / len(test_dataloader)
        avg_test_kl = test_kl / len(test_dataloader)
        ...
        # --- Write losses to CSV ---
        csv_path = "hierarchical_finetune_losses.csv"
        ...
        if (epoch + 1) % 5 == 0:
            ckpt_path = f"vae_hierarchical_finetune_epoch{epoch+1}.pt"
    ...
    with open("hierarchical_finetune_history.json", "w") as f:
        json.dump(history, f, indent=2)

    torch.save(hierarchical_vae.state_dict(), "vae_hierarchical_finetuned.pt")
    print("Fine-tuning complete! Model saved as vae_hierarchical_finetuned.pt")
